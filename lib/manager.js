...
var Manager = module.exports = function(opts) {
  ...
  var allNeurons = [self.inputNeuron].concat(self.neurons).concat([self.output]).map(function(neuron) {
    return {id: neuron.id};
  });
...
Manager.prototype.test = function(iterations, msPerIteration, testSample, done) {
  ...
  var runIteration = function() {
    ...
    self.inputNeuron.hold({id: 'INPUT', signal: testSample ? M.multiply(sample.input, 3) : sample.input})
