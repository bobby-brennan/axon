...
var DECAY_RATE = .7;
var EVOLVE_RATE = .2;
var EVOLVE_SCALE = 1.4; // must be > 1
...
Neuron.prototype.adjustTransform = function(input, transformed) {
  ...
  var trans = self.state.transforms[input.id];
  var maxTries = 5;
  var movementIncr = (input.distance * M.norm(self.state.hidden)) / maxTries;
...
Neuron.prototype.signal = function(input) {
  ...
  if (!self.state.holding) {
    ...
    if (hiddenNorm < .1) {
      self.state.hidden = M.multiply(stateNormalized, .5);
    } else {
      ...
      var scale = (1 - distance) * EVOLVE_SCALE * hiddenNorm;
