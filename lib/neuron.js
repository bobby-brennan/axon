...
Neuron.prototype.transform = function(input) {
  ...
  if (!trans) {
    trans = self.state.transforms[input.id] = M.ones(input.signal.size()[0], self.options.dimensions.hidden);
  }
...
Neuron.prototype.signal = function(input) {
  var self = this;
  self.log('signal from', input.id, input.signal._data);
  ...
  if (!self.state.holding) {
    var contribState = M.multiply(state, .1);
    var contribHidden = M.multiply(self.state.hidden, .9);
  ...
  if (self.state.holding) console.log('dist', input.distance);
}
...
Neuron.prototype.maybeFire = function(callback) {
  ...
  if (norm > 1.0) {
    ...
    self.fire(self.state.hidden, function() {
      if (!self.state.holding) self.state.hidden = M.multiply(self.state.hidden, .3);
