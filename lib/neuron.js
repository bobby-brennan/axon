...
Neuron.prototype.adjustTransform = function(input, transformed) {
  ...
  var trans = self.state.transforms[input.id];
  var maxTries = 5;
  var movementIncr = (input.distance / 2) / maxTries;
...
Neuron.prototype.signal = function(input) {
  ...
  self.io.socket.emit('signal', {
    ...
    norm: input.norm,
    normalized: M.divide(input.signal, input.norm)._data,
  ...
  if (!self.state.holding) {
    ...
    hiddenNorm = Math.max(hiddenNorm, .1);
    var hiddenNormalized = M.divide(self.state.hidden, hiddenNorm);
    var stateNormalized = M.divide(state, stateNorm);
    var distance = Neuron.getDistance(hiddenNormalized, stateNormalized);
    var contribHidden = M.multiply(hiddenNormalized, 1 - EVOLVE_RATE);
    var contribState = M.multiply(stateNormalized, EVOLVE_RATE);
    var scale = (1 - distance) * 2 * hiddenNorm;
    self.state.hidden = M.multiply(M.add(contribHidden, contribState), scale);
    //self.state.hidden = M.add(self.state.hidden, M.multiply(state, EVOLVE_RATE / stateNorm));
  }
...
Neuron.prototype.maybeFire = function(callback) {
  ...
  var decayAndReset = function() {
    if (!self.state.holding) self.state.hidden = M.multiply(self.state.hidden, 1 - DECAY_RATE);
  ...
  if (norm > 1.0) {
    ...
    self.fire(self.state.hidden, decayAndReset);
  } else {
    decayAndReset();
  }
}
